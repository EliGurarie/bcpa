...
#' 
#' @param x a \code{windowsweep} object, i.e. the output of the 
#' \code{\link{WindowSweep}} function.
#' @param type whether to plot smooth or flat bcpa output
#' @param threshold for smooth BCPA, this is the minimum number of windows that 
#' must have identified a given changepoint to be illustrated.
#' @param clusterwidth for flat BCPA, this is the temporal range within which 
#' change points are considered to be within the same cluster. 
#' @param col.cp,col.mean,col.sd color of the vertical change points, mean 
...
#' @param rho.where where to place the legend for the time-scale / 
#' auto-correlation.  Can be one of "nowhere", "top", "bottom", "left", "right", 
#' "topleft", "topright", "bottomright", "bottomleft"
#' @param mu.where where (and whether) to place the legend box for the mean - 
...
plot.bcpa <- function (x, type = c("smooth", "flat")[1], threshold = 3, 
                       clusterwidth = 1, col.cp = rgb(0.5, 0, 0.5, 0.5), 
                       pt.cex = 0.5, legend = TRUE, rho.where = "topleft", 
                       mu.where = "nowhere", col.sd="red", col.mean="black",
                       ...) 
  {
    windowsweep <- x
    x <- windowsweep$x
    ...
    if (type == "flat") {
      ...
      for(i in 1:nrow(phases))
        closematch[i] <- which(abs(t-phases$t0[i]) == 
                                 min(abs(t-phases$t0[i])))[1]
      ...
      segments(phases$t0.POSIX, phases$mu.hat + phases$s.hat, 
               phases$t1.POSIX, phases$mu.hat + phases$s.hat, col = col.sd, 
               lwd = 1.5)
      abline(v = phases$t0.POSIX[-1], lwd = breaks$size/max(breaks$size) * 4, 
             col = col.cp)
    }
    if (legend) {
